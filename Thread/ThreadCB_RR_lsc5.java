...
public class ThreadCB extends IflThreadCB 
{
    ...
    */
    public void do_kill()
    {
        ...
        if (currentStatus == GlobalVariables.ThreadKill) {
            ThreadCB.dispatch();
            return;
        }
    ...
    *    */
    public void do_suspend(Event event)
    {
        ...
        if (currentStatus == GlobalVariables.ThreadKill || 
            currentStatus == GlobalVariables.ThreadReady) {
            ThreadCB.dispatch();
            return;
        }
    ...
    */
    public void do_resume()
    {
        ...
        if (currentStatus == GlobalVariables.ThreadKill || 
            currentStatus == GlobalVariables.ThreadReady ||
            currentStatus == GlobalVariables.ThreadRunning) {
            ThreadCB.dispatch();
            return;
        }
    ...
    */
    public static void handleTimerInterrupt(){
        atLog(null, "Handle timer interrupt.");
    ...
    */
    public static int do_dispatch()
    {
        ...
        if (selectedThread.getStatus() == GlobalVariables.ThreadReady) {
            selectedThread.setRespondTime(HClock.get() - selectedThread.getCreationTime());
    ...
    */
    private void do_statistics(long terminationTime){
        ...
        long totalT = 0;
        long totalRespondT = 0;
        int totalRespondedThread = 0;
        // minRunningTime = null;
        // maxRunningTime = null;
        for (ThreadStat ts: threadStatsList) {
            // MyOut.print(ts.thread, ts.toString());
            totalT += ts.runningTime; 
            if (ts.thread.getRespondTime() != null) {
                totalRespondT += ts.thread.getRespondTime();
                totalRespondedThread ++;
            }
            totalNumberKilled ++;
            // //Calculate minimum running time
            // if (minRunningTime == null || ts.runningTime < minRunningTime) {
            //     minRunningTime = ts.runningTime;
            // }
            // //Calculate maximum running time
            // if (maxRunningTime == null || ts.runningTime > maxRunningTime) {
            //     maxRunningTime = ts.runningTime;
            // }
        ...
        //Log output
        MyOut.print(this, 
            // "Min service time: " + minRunningTime + 
            // "\n\tMax service time: " + maxRunningTime + 
            // "\n\tAverage service time: " + (double)totalT / totalNumberKilled +
            "\n\tAverage respond time: " + (double)totalRespondT / totalRespondedThread +
            "\n\tThroughput: " + totalNumberKilled);
            // "\n\tTotal # of threads: " + totalNumberKilled + 
            // "\n\tCurrent System Time: " + ts.terminationTime);

    }

    // private void printStat(long totalT, long totalCPUT, int totalNumberKilled, long terminationTime){
    //     //Log output
    //     MyOut.print(new Long(terminationTime), "Min service time: " + minRunningTime + 
    //         "\n\tMax service time: " + maxRunningTime + 
    //         "\n\tAverage service time: " + (double)totalT / totalNumberKilled +
    //         "\n\tthroughput: " + (double)totalNumberKilled / totalT+
    //         "\n\tTotal # of threads: " + totalNumberKilled + 
    //         "\n\tCurrent system clock: " + terminationTime);
    // }
