...
public class ThreadCB extends IflThreadCB 
{
    ...
    *    */
    public void do_suspend(Event event)
    {
        ...
        if (currentStatus == GlobalVariables.ThreadRunning) {
            ...
            readyToRunThreads.remove(this);
            atLog(null, "\t Thread # remained in ready queue: " + readyToRunThreads.size());
    ...
    */
    public static int do_dispatch()
    {
        ...
        if (readyToRunThreads.size() == 0 && 
            selectedThread.getStatus() == GlobalVariables.ThreadReady) {
            atLog(null, "\tReschedule thread to ready queue");
            readyToRunThreads.add(selectedThread);
        }
        if (selectedThread.getStatus() >= ThreadWaiting) {
            atLog(null, "\tThread waiting, abort dispatch.");
